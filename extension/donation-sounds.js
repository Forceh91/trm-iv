...
module.exports = function(nodecg) {
	...
	nodecg.log.info("[SECRET SOUNDS] Loaded", secretSounds.value.length, "secret sounds");
	...
	function addSound(data) {
		...
		if (typeof data.imageAssetName !== "string")
			return;

		if (typeof data.soundAssetName !== "string")
			return;
	...
	function removeSound(id) {
		...
		if (typeof soundToRemove !== "string")
			return;
	...
	function getSoundAndImageForAmount(amount, callback) {
		...
		if ((secretSound = secretSounds.value.find(sound => sound.amount == amount)) != null) {
			imageFile = images.value.find(image => image.name === secretSound.imageName);
			soundFile = sounds.value.find(sound => sound.name === secretSound.soundName);
			secretSound.used++;
		} else {
